...
| 항목 | dockerd (Moby) | containerd |
|---|---|---|
| docker compose 호환 | ✅ 완전 호환 | ⚠️ nerdctl compose — 미구현 기능 다수 |
| CLI | `docker`, `docker compose` 그대로 | `nerdctl`, `nerdctl compose`로 교체 필요 |
| 안정성 | ✅ Rancher Desktop 1.22+ 주요 버그 수정 | ✅ 안정적이나 compose 엣지케이스 버그 있음 |
| 리소스 | Lima VM이 병목 — 런타임 차이 미미 (MB 단위) | Lima VM이 병목 — 런타임 차이 미미 |
| 권장 용도 | **일반 개발, docker compose 기반 인프라** | Kubernetes/k3s 연동, 로컬 K8s 개발 |

**지금처럼 docker compose로 shared_caddy + shared_postgres + 앱 컨테이너를 관리하는 구조라면 dockerd가 맞다.**  
containerd로 바꿔봤자 nerdctl compose로 명령어를 바꿔야 하고, 미묘한 호환 차이만 생긴다.
...
2. `Preferences → Container Engine → General` → **dockerd (moby)** 선택
